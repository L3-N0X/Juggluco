...
fun MirrorConnectionEditScreen(
    ...
    SettingsDetailScaffold(
        title = screenTitle,
        subtitle = stringResource(R.string.settings_desc_mirror_edit),
        onNavigateBack = onNavigateBack
    ) {
        // IDENTIFICATION & HOST CARD
        SettingsCard(
            title = "Target Host & Address",
            icon = Icons.Default.Devices,
            categorySubtitle = "NETWORK TARGET"
        ) {
            OutlinedTextField(
                value = label,
        ...
        // ROLE & PROTOCOL CARD
        SettingsCard(
            title = "Device Role & Direction",
            icon = Icons.Default.Sync,
            categorySubtitle = "ROLE IN RELAY"
        ) {
            Row(
                modifier = Modifier
                    .fillMaxWidth()

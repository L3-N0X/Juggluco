...
fun GlucoseGraph(
    ...
    // Filter points in the active window (with a slight margin for smooth continuous spline joins)
    ...
    // Dynamic Y scale bounds (never clip high readings)
    val minY = 40f
    val highestReading = visiblePoints.maxOfOrNull { it.valueMgDl } ?: 180f
    val maxY = max(240f, ((highestReading + 30f) / 20f).roundToInt() * 20f)

    // Current latest state references for gesture handler without recomposition churn
    ...
    // Reusable drawing objects to avoid garbage collection stutter during 60/120fps pan
    ...
    Box(
        ...
        if (readings.isEmpty()) {
            ...
            Canvas(
                modifier = Modifier
                    ...
                    .pointerInput(Unit) {
                        ...
                        awaitEachGesture {
                            ...
                            var wasInspectingOnDown = (inspectedPoint != null)
                            ...
                            // Touch release / Gestures conclusion
                            if (!isHorizontalDrag && !isMultiTouch && totalDragX < touchSlop && totalDragY < touchSlop) {
                                // Tap detection: inspect point near touch
                                val touchX = down.position.x
                                val touchY = down.position.y
                                val vp = currentViewportState.value

                                // Check if an amount/log pin was tapped along bottom
                                ...
                                if (abs(vx) > 350f) {
                                    flingJob = coroutineScope.launch {
                                        ...
                                        while (abs(currentVelocity) > 80f && isActive) {
                                            ...
                                            vp.panBy(timeDelta)
                                            currentVelocity *= 0.92f // friction decay
                                            delay(16)
                                        }
                                    }
                                }
                ...
                val paddingLeft = 14.dp.toPx()
                val paddingRight = 62.dp.toPx()
                val paddingTop = 22.dp.toPx()
                val paddingBottom = 42.dp.toPx()
                ...
                // 1. Draw Target In-Range Band (Shaded zone)
                val targetTopY = valueToY(targetHigh)
                val targetBottomY = valueToY(targetLow)
                drawRect(
                    color = clinicalColors.targetRangeShade,
                ...
                drawLine(
                    color = clinicalColors.inRange.copy(alpha = 0.55f),
                ...
                gridTextPaint.textSize = 10.sp.toPx()
                targetHighTextPaint.textSize = 10.sp.toPx()

                while (gridVal < maxY) {
                    val yPos = valueToY(gridVal)
                    drawLine(
                        color = clinicalColors.graphGrid,
                ...
                drawTimeAxisGrid(
                    ...
                    gridColor = clinicalColors.graphGrid,
                ...
                // 6. Draw Layer: Real-Time Continuous Stream Curve (Cubic Spline)
                ...
                // 7. Draw Layer: Calibrated Stream Curve (Distinct cyan/teal dashed curve)
                ...
                if (selected != null) {
                    ...
                    drawCircle(color = surfaceColor, radius = 6.5f, center = Offset(sx, sy))
                    drawCircle(color = statusColor, radius = 5.0f, center = Offset(sx, sy))
                }
            }

            // 11. Floating Detailed Inspection HUD Card
            AnimatedVisibility(
            ...
            // 12. Floating "Jump to Now" Pill if viewing past data
    ...
    for (seg in segments) {
        if (seg.size < 2) {
            if (seg.size == 1) {
                ...
                drawCircle(color = surfaceColor, radius = 5f, center = Offset(px, py))
                drawCircle(color = clinicalColors.inRange, radius = 3.5f, center = Offset(px, py))
            }
            continue
        }
        ...
        // Smooth continuous curve stroke
        drawPath(
            path = linePath,
        ...
        for (pt in seg) {
            ...
            drawCircle(color = surfaceColor, radius = 4.2f, center = Offset(px, py))
            drawCircle(color = ptColor, radius = 2.8f, center = Offset(px, py))
        }
    }
}
...
private fun DrawScope.drawCalibratedStreamLayer(
    ...
    val caliColor = Color(0xFF06B6D4) // Cyan / Teal accent for calibrated stream
    var started = false
    ...
    for (i in points.indices) {
        ...
        drawCircle(color = surfaceColor, radius = 3.8f, center = Offset(px, py))
        drawCircle(color = caliColor, radius = 2.5f, center = Offset(px, py))
    }
...
private fun DrawScope.drawScansLayer(
    ...
    for (pt in points) {
        ...
        path.reset()
        path.moveTo(px, py - 6.5f)
        path.lineTo(px + 6.5f, py)
        path.lineTo(px, py + 6.5f)
        path.lineTo(px - 6.5f, py)
        path.close()

        drawPath(path = path, color = scanColor)
        drawPath(path = path, color = surfaceColor, style = Stroke(width = 1.4f))
    }
}
...
private fun findTappedLog(
    ...
    // Only inspect logs if touched near the bottom timeline strip
